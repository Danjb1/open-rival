...
// Largest image in images.dat is 128 x 128 (Increase size if scanning an unknown file)
...
void reset_image(void) {
	// Color index 255 (0xff) is new written by the code (in images.dat)
...
int main() {
	...
	for (uint8_t* code = data + 4; code < data + size; ++cnt) {
		int read_from_esi;
		uint8_t* const code_end = find_end(code, data + size, &read_from_esi);
		reset_image();
		memset(team_color, 0, sizeof team_color);
		call_code(code);
		int w = 0, h = 0;
		...
		char filename[32];
		if (!read_from_esi) {
			snprintf(filename, sizeof(filename), "images/img%08X.tga", code - data);
			write_image(filename, w, h);
		}
		else {
			const int num_colors_per_team = sizeof team_color;
			const int num_teams = 1; // Increase to 8 to output all team colours
			const int col_start = 160;
			for (int team = 0; team < num_teams; ++team) {
				for (int col = 0; col < num_colors_per_team; ++col) {
					team_color[col] = (uint8_t)(col_start + team * num_colors_per_team + col);
				}
				call_code(code); // Will only overdraw already drawn pixels, so no need to clear
				snprintf(filename, sizeof(filename), "images/img%08X_%d.tga", code - data, team);
				write_image(filename, w, h);
			}
		}
#endif
