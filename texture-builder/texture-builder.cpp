...
void createTextureAtlas(
    ...
    writeImage("./textures/" + definitionFilename.replace_extension(".tga").string(),
            texture,
            palette);

    // Save the atlas definition
    writeAtlas("./textures/" + definitionFilename.replace_extension(".atlas").string(),
            imagesByKey,
            imagePlacements);
}
...
int main(int argc, char *argv[]) {
    ...
    for (const fs::directory_entry& entry : fs::directory_iterator(definitionsDir)) {
        ...
        try {
            ...
            // Create texture
            if (atlasMode) {
                createTextureAtlas(path.filename(), images, palette);
            } else {
                createSpritesheetTexture(path.filename(), images, palette);
            }
