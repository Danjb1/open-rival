...
Resources::Resources(ApplicationContext& context)
    ...
    , projectileSpritesheets(initProjectileSpritesheets())
...
std::unordered_map<std::string, Spritesheet> Resources::initProjectileSpritesheets()
{
    ...
    {
        spritesheets.emplace(std::piecewise_construct,
                std::forward_as_tuple(textureNames[i]),
                std::forward_as_tuple(textures.at(i), RenderUtils::entityWidthPx, RenderUtils::entityHeightPx, 0));
    }
...
const Spritesheet& Resources::getProjectileSpritesheet(const std::string& name) const
{
