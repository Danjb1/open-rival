...
#pragma warning(push)
#pragma warning(disable : 4242 4244 4267 4456)
...
void midi_track::add_event(const midi_event& p_event)
{
    ...
    if (m_events.size())
    {
        ...
        {
            if ((*(it - 1)).m_timestamp <= p_event.m_timestamp)
                break;
            --it;
        }
    }
...
void tempo_map::add_tempo(unsigned p_tempo, unsigned long p_timestamp)
{
    ...
    {
        if ((*(it - 1)).m_timestamp <= p_timestamp)
            break;
        --it;
    }
...
void midi_meta_data::assign_bitmap(
        std::vector<std::uint8_t>::const_iterator const& begin, std::vector<std::uint8_t>::const_iterator const& end)
{
        ...
        unsigned clean_flags) const
{
    ...
    if (m_form == 2)
    {
        ...
        {
            if (i != subsong)
                track_positions[i] = m_tracks[i].get_count();
        }
    }
    ...
    for (;;)
    {
        ...
        for (unsigned i = 0; i < track_count; ++i)
        {
            if (track_positions[i] >= m_tracks[i].get_count())
                continue;
    ...
    for (unsigned long i = 0, j = m_tracks.size(); i < j; ++i)
    {
        ...
        for (unsigned long k = 0, l = source_track.get_count(); k < l; ++k)
        {
            ...
            if (event.m_type == midi_event::program_change
                || (event.m_type == midi_event::control_change && (event.m_data[0] == 0 || event.m_data[0] == 0x20)))
            {
...
void midi_container::scan_for_loops(bool p_xmi_loops, bool p_marker_loops, bool p_rpgmaker_loops, bool p_touhou_loops)
{
    ...
    if (p_rpgmaker_loops)
    {
        ...
        for (unsigned long i = 0; i < m_tracks.size(); ++i)
        {
            ...
            {
                const midi_event& event = track[j];
                if (event.m_type == midi_event::control_change && (event.m_data[0] == 110 || event.m_data[0] == 111))
                {
    ...
    if (p_xmi_loops)
    {
        for (unsigned long i = 0; i < m_tracks.size(); ++i)
        {
            ...
            {
                const midi_event& event = track[j];
                if (event.m_type == midi_event::control_change && (event.m_data[0] >= 0x74 && event.m_data[0] <= 0x77))
                {
