...
void extractImage(
    ...
    int next = readShort(data, offset);
    if (next != 0x03) {
        offset += 2;
    }
}

void extractImages(const uint8_t* data) {

    int offset = OFFSET_TITLE;

    // Title image
    extractImage(data, offset, PALETTE_TITLE);

    // Menu backgrounds
    for (int i = 0; i < 9; i++) {
        extractImage(data, offset, PALETTE_MENU);
    }

    // "Hire Troops" background
    extractImage(data, offset, PALETTE_HIRE_TROOPS);
    ...
    for (int i = 0; i < 2; i++) {
        extractImage(data, offset, PALETTE_LOADING);
    }
    ...
    // Game icons
    for (int i = 0; i < 592; i++) {
        extractImage(data, offset, PALETTE_GAME);
    }
}
...
int main(int argc, char *argv[]) {
    ...
    // Read the file
    uint32_t size;
    uint8_t* data = (uint8_t*)read_file("Interfac.dat", &size);
    if (!data) {
        printf("Error reading Interfac.dat");
        return 1;
    }
