...
#define MIDI_CONTAINER_H

/**
 * Classes to store and serialize MIDI data.
 *
 * Based on the midi_processing library by Christopher Snowhill:
 * https://github.com/kode54/midi_processing/blob/master/midi_container.cpp
 *
...
namespace Rival {
    ...
    struct midi_event {
        ...
        std::uint8_t m_data[max_static_data_count];
        std::vector<std::uint8_t> m_ext_data;
...
#pragma warning(pop)
    ...
    private:
        unsigned m_form;
        unsigned m_dtx;
        std::vector<std::uint64_t> m_channel_mask;
        std::vector<tempo_map> m_tempo_map;
        std::vector<midi_track> m_tracks;

        std::vector<std::uint8_t> m_port_numbers;
        ...
        void limit_port_number(T& number) {
            ...
            m_port_numbers.push_back((const std::uint8_t) number);
...
#pragma warning(pop)
        ...
        /*
         * These functions are really only designed to merge and later remove System
         * Exclusive message dumps
         */
