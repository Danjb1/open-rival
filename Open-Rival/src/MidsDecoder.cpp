...
namespace Rival {
    ...
    bool MidsDecoder::is_mids(std::vector<uint8_t> const& p_file) {
        ...
        if (size < 8 || (p_file.size() < size + 8))
            return false;
        if (p_file[8] != 'M' || p_file[9] != 'I' || p_file[10] != 'D'
                || p_file[11] != 'S' || p_file[12] != 'f' || p_file[13] != 'm'
                || p_file[14] != 't' || p_file[15] != ' ')
            return false;
        return true;
    }
    ...
    bool MidsDecoder::process_mids(
        ...
        {
            midi_track track;
            track.add_event(midi_event(0, midi_event::extended, 0, end_of_track,
                    _countof(end_of_track)));
        ...
        for (unsigned i = 0; i < segment_count; ++i) {
            ...
            while (it != segment_end && it != body_end) {
                ...
                if (event >> 24 == 0x01) {
                    ...
                    p_out.add_track_event(0,
                            midi_event(current_timestamp, midi_event::extended, 0,
                    ...
                    if (event_code >= 0x8 && event_code <= 0xE) {
                        ...
                        track.add_event(midi_event(current_timestamp,
                                (midi_event::event_type)(event_code - 8),
                                event & 0x0F, buffer, bytes_to_write));
                    }
                }
            }
        }

        track.add_event(midi_event(current_timestamp, midi_event::extended, 0,
                end_of_track, _countof(end_of_track)));
