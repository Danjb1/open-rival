...
void MapBorderRenderer::render() const
{
    ...
    // Render
    glDrawElements(renderable.getDrawMode(), numSegments * renderable.getIndicesPerSprite(), GL_UNSIGNED_INT, nullptr);
}
        ...
        std::vector<GLfloat>& positions, std::vector<GLfloat>& texCoords, int raceOffset, int mapWidth, int mapHeight)
{
    for (int tileY = 1; tileY < mapHeight; ++tileY)
    {
        addDataToBuffers(positions, texCoords, raceOffset + txIndexRight, mapWidth - 1.0f, static_cast<float>(tileY));
        numSegments++;
    }
}
        ...
        std::vector<GLfloat>& positions, std::vector<GLfloat>& texCoords, int raceOffset, int mapWidth, int mapHeight)
{
    for (int tileX = 1; tileX < mapWidth - 1; ++tileX)
    {
        addDataToBuffers(positions, texCoords, raceOffset + txIndexBottom, static_cast<float>(tileX), mapHeight - 0.5f);
        numSegments++;
    }
}
